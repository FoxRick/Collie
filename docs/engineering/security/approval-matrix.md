...
| Memory (`remember`) | Additive new facts, people, and dates are prompt-free: reversible local writes, journaled for undo in Settings, with the visible "remember" pill. Forgetting a memory, or rewriting one that already holds a different value, requires fresh approval — a change in disguise, not an addition. Unknown memory kinds fail closed and ask. *(Decision 2026-08-20: keep additive writes approval-free — reversible + journaled, with forgets/conflicts already gated; the higher-risk Dream full-file rewrite path is gated by propose-before-apply. Closes #62.)* |
