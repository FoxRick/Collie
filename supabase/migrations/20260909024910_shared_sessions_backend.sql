...
begin
 ...
 if p_command='create_organization' then
   ...
   -- Release any OTHER expired run lease in this session so the next claim is
   -- not blocked by the partial unique index on the stale still-'leased' row
   -- (which would otherwise surface a raw 23505 instead of a clean RUN_* reply).
   update public.collaboration_runs set status='needs_reconciliation',lease_token=null,lease_expires_at=null
     where session_id=(select session_id from public.collaboration_runs where run_id=v_run)
       and run_id<>v_run and status='leased' and lease_expires_at<now();
