...
def trusted_goal_start_requested(message_metadata: Mapping[str, Any] | None) -> bool:
    """True only for the command layer's complete, trusted ``/goal`` marker.

    Runtime behavior may use :func:`explicit_goal_requested` for compatibility
    with older command envelopes.  Goal mutation is deliberately stricter: both
    fields must be present so user text or a partial metadata copy cannot grant
    permission to create or replace persisted goal state.
    """
    if not message_metadata:
        return False
    return (
        message_metadata.get("goal_requested") is True
        and str(message_metadata.get("original_command") or "").strip() == GOAL_COMMAND
    )
