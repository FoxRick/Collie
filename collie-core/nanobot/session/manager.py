...
class SessionManager:
    ...
    def _cache_put(self, key: str, session: Session) -> None:
        """Cache a session, evicting the oldest entry once the cap is reached.

        Sessions are persisted to disk on every ``save``, so evicting from the
        cache only costs a later re-read from JSONL — it must never grow
        without bound over the process lifetime.
        """
        self._cache[key] = session
        if len(self._cache) <= _CACHE_MAX_SESSIONS:
            return
        while len(self._cache) > _CACHE_MAX_SESSIONS:
