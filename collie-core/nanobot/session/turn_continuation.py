...
async def maybe_continue_turn(ctx: Any) -> bool:
    ...
    try:
        # put_nowait: the bounded pending queue must never let a full queue
        # hang the turn forever on a continuation push.
        ctx.pending_queue.put_nowait(continuation)
    except asyncio.QueueFull:
        logger.warning(
            "Pending queue full; returning the current response without continuation"
        )
        return False

    # Commit continuation bookkeeping only after the queue accepted the next
    # slice. QueueFull must leave the visible response and saved history intact.
