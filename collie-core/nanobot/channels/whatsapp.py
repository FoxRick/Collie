...
class WhatsAppChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # Collie: optional async hooks so the desktop shell can render the
        # login QR code and connection status in the GUI (no terminal).
        self.on_qr: Any | None = None
        self.on_status: Any | None = None
    ...
    async def _emit_status(self, status: str) -> None:
        ...
        try:
            ...
            self.logger.debug("WhatsApp status hook failed", exc_info=True)
    ...
    def _register_handlers(
        ...
        *,
        ...
        async def _on_qr(_: Any, qr_data: bytes) -> None:
            payload = qr_data.decode("utf-8", errors="replace")
            if self.on_qr is not None:
                try:
                    await self.on_qr(payload)
                    return
                except Exception:
                    self.logger.debug("WhatsApp QR hook failed", exc_info=True)
            import segno
