...
class _QueuedTelegramUpdate:
    ...
    kind: Literal["command", "message", "callback"]
    update: Update
    context: Any
...
class TelegramChannel(BaseChannel):
    ...
    # Collie has no slash commands — natural language only (plus the two
    # Telegram builtins users expect).
    BOT_COMMANDS = [
        BotCommand("start", "Say hi to Collie"),
        BotCommand("help", "What Collie can do"),
        BotCommand("new", "Start a fresh conversation"),
        BotCommand("compact", "Summarize older context"),
        BotCommand("status", "Show session status"),
        BotCommand("stop", "Stop the current response"),
        BotCommand("agents", "Show specialist agents"),
        BotCommand("skills", "Show reusable skills"),
    ]
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._fatal_error: str | None = None  # surfaced to the UI via status
    ...
    async def send_delta(
        ...
        *,
        ...
        key = stream_id or f"chat:{chat_id}"
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! Collie here — your desktop buddy, now in your pocket.\n\n"
            "Just send me a message like you'd text a friend. I'm all ears! 🐕"
        )
    ...
    async def _drain_ordered_updates(self, key: str) -> None:
        ...
        try:
                ...
                for item in batch:
                    try:
                        if item.kind == "command":
                            ...
                            await self._process_callback_query(item.update, item.context)
                        else:
                        ...
                        # Re-enqueue failed updates once so a transient hiccup
                        # does not silently swallow a user message.
                        if item.retries_left > 0 and self._running:
                            item.retries_left -= 1
                            self._inbound_buffers.setdefault(key, []).append(item)
            ...
            # A message may have been enqueued between our final emptiness
            # check and this finally: hand it to a fresh worker instead of
            # orphaning it forever (the old worker would otherwise be popped
            # while the message still waits).
            if self._running and self._inbound_buffers.get(key):
                self._inbound_workers[key] = asyncio.create_task(
                    self._drain_ordered_updates(key)
                )
            elif not self._inbound_buffers.get(key):
    ...
    def _on_polling_error(self, exc: Exception) -> None:
        ...
        if isinstance(exc, (NetworkError, TimedOut)):
            ...
            return
        if self._is_fatal_telegram_error(exc):
            self.logger.error("FATAL Telegram polling error: {}", summary)
    ...
    async def _on_error(self, update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        elif self._is_fatal_telegram_error(context.error):
            self.logger.error("FATAL Telegram error: {}", summary)
            self._fatal_error = summary
        else:
            self.logger.error("error: {}", summary)

    @staticmethod
    def _is_fatal_telegram_error(exc: Exception) -> bool:
        """Token/authorization-level failures that no retry will fix."""
        try:
            from telegram.error import Forbidden, Unauthorized
        except Exception:
            Forbidden = Unauthorized = ()  # noqa: N806 - imported names
        if isinstance(exc, (Unauthorized, Forbidden)):
            return True
        text = TelegramChannel._format_telegram_error(exc).lower()
        return any(
            marker in text
            for marker in (
                "unauthorized",
    ...
    async def _on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self.is_allowed(sender_id):
            return
        if not self._running:
            await self._process_callback_query(update, context)
            return
        # Route through the per-session ordering queue so a button tap cannot
        # overtake queued messages for the same conversation.
        self._enqueue_ordered_update(
            kind="callback",
            update=update,
            context=context,
    ...
    async def _process_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Process a queued inline button tap as a user message."""
        query = update.callback_query
        user = update.effective_user
        chat_id = query.message.chat_id if query.message else None
        sender_id = self._sender_id(user)
