...
def generate_code(
    ...
    with _LOCK:
        ...
        _save(data)
        logger.info("Generated pairing code {}-XXXX for {}@{}", raw[:4], sender_id, channel)
        return code
...
def approve_code(code: str) -> tuple[str, str] | None:
    ...
    with _LOCK:
        ...
        _save(data)
        logger.info("Approved pairing code {}-XXXX for {}@{}", code[:4], sender_id, channel)
...
def deny_code(code: str) -> bool:
    ...
    with _LOCK:
        ...
        if code in pending:
            del pending[code]
            _save(data)
            logger.info("Denied pairing code {}-XXXX", code[:4])
            return True
        return False
