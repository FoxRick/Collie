...
FinalizeOnMaxIterations = bool | Callable[[], bool]
...
class AgentRunSpec:
    ...
    finalize_on_max_iterations: FinalizeOnMaxIterations = True
...
class AgentRunner:
    ...
    async def _run_core(
        ...
        for iteration in range(spec.max_iterations):
            ...
            if drained_after_max_iterations:
                had_injections = True
            final_content = None
            if self._finalize_after_max_iterations_enabled(spec):
    ...
    @staticmethod
    def _finalize_after_max_iterations_enabled(spec: AgentRunSpec) -> bool:
        decision = spec.finalize_on_max_iterations
        if not callable(decision):
            return decision
        try:
            return bool(decision())
        except Exception:
            # A dynamic policy must never turn a recoverable budget boundary
            # into a failed turn. Prefer the normal no-tools finalization pass.
            logger.exception("Dynamic max-iteration finalization policy failed")
            return True
        ...
        if spec.authorizer is not None:
            ...
            try:
                await spec.authorizer.authorize(
                    execution_context, tool_call, tool, params
                )
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                detail = str(exc) or "The action was not authorized."
                event = {
                    "name": tool_call.name,
                    "status": "denied",
                    "detail": detail[:120],
