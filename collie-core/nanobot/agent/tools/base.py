...
class Tool(ABC):
    ...
    def permission_action(self) -> str:
        """Stable policy action. Empty means the central classifier decides."""
        return ""

    @property
    def permission_resource(self) -> str:
        """Static resource hint. Dynamic tools should implement permission_request."""
        return self.name

    @property
    def permission_risk(self) -> str:
        """Risk hint: read, local_write, external_write, sensitive, or destructive."""
        return "read" if self.read_only else ""
