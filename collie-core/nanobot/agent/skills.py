...
class SkillsLoader:
    ...
    def load_skill(self, name: str) -> str | None:
        ...
        The name must be a plain slug that matches one of the enumerated
        skill directories; anything else (``..``, separators, absolute or
        drive-relative paths) is rejected before touching the filesystem.
        ...
        if not name or not re.fullmatch(r"[A-Za-z0-9._-]+", name):
            return None
        known = {entry["name"] for entry in self.list_skills(filter_unavailable=False)}
