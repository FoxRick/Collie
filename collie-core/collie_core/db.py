...
- Connection plumbing, schema and migrations live in this module. One storage
  domain per module lives in :mod:`collie_core.db_domains`; those modules are
  mixed into :class:`CollieDB`, so ``db.<method>`` stays the public interface.
"""
...
from collie_core.db_domains import (
    ApprovalsDomain,
    ArtifactsDomain,
    AutomationsDomain,
    ChecklistsDomain,
...
from collie_core.db_primitives import collie_home, new_id, utc_now

__all__ = ["CollieDB", "collie_home", "new_id", "utc_now"]
...
class CollieDB(
    ...
    The connection, schema, migrations and shared row helpers live here; each
    storage domain is mixed in from :mod:`collie_core.db_domains` and owns its
    own tables. Callers keep using ``db.<method>`` for everything.
    """
    ...
    # -- shared status vocabulary (checklists and run tasks) ------------------
