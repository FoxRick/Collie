...
@pytest.mark.asyncio
async def test_queue_full_keeps_current_response_and_continuation_state() -> None:
    meta = {GOAL_STATE_KEY: {"status": "active", "objective": "Finish safely."}}
    messages = [
        {"role": "system", "content": "system"},
        {"role": "user", "content": "start"},
        {"role": "assistant", "content": "current answer"},
    ]
    pending: asyncio.Queue[InboundMessage] = asyncio.Queue(maxsize=1)
    ...
    assert pending.get_nowait() is existing
    assert ctx.final_content == "current answer"
    assert ctx.all_messages is messages
    assert ctx.suppress_response is False
    assert INTERNAL_CONTINUATION_PENDING_META not in msg.metadata
