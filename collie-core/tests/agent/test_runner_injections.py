...
async def test_waiting_dispatch_does_not_replace_active_pending_queue(tmp_path):
    ...
    loop = _make_loop(tmp_path)
    session_key = "cli:c"
    lock = loop._session_lock_entry(session_key).lock
    await lock.acquire()
...
@pytest.mark.asyncio
async def test_steer_session_adds_followup_to_active_direct_queue(tmp_path):
    loop = _make_loop(tmp_path)
    session_key = "collie:conversation"
    pending = asyncio.Queue(maxsize=20)
    loop._pending_queues[session_key] = pending

    accepted = await loop.steer_session(
        session_key,
        "Use the shorter introduction.",
        channel="collie",
    ...
    assert queued.content == "Use the shorter introduction."
    assert queued.session_key == session_key


@pytest.mark.asyncio
async def test_process_direct_publishes_and_cleans_up_steering_queue(tmp_path):
    loop = _make_loop(tmp_path)
    session_key = "collie:conversation"
    loop._connect_mcp = AsyncMock()  # type: ignore[method-assign]

    async def fake_process(_message, **kwargs):
        pending = kwargs["pending_queue"]
        assert loop._pending_queues[session_key] is pending
        return None
