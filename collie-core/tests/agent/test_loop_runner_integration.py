...
@pytest.mark.parametrize(
    ("metadata", "session_metadata", "expected"),
    [
        ({"goal_requested": True, "original_command": "/goal"}, {}, True),
        ({}, {"goal_state": {"status": "active", "objective": "x"}}, True),
        ({"goal_requested": True}, {}, False),
        ({"original_command": "/goal"}, {}, False),
        ({}, {}, False),
    ],
)
async def test_runner_binds_goal_permission_from_trusted_turn_or_active_session(
    tmp_path,
    metadata,
    session_metadata,
    expected,
    ...
    async def chat_with_retry(**_kwargs):
        assert goal_mutation_allowed() is expected
        return LLMResponse(content="done", tool_calls=[], usage={})

    loop.provider.chat_with_retry = AsyncMock(side_effect=chat_with_retry)
    loop.tools.get_definitions = MagicMock(return_value=[])
    session = SimpleNamespace(key="cli:goal", metadata=session_metadata)
...
@pytest.mark.asyncio
async def test_goal_completed_on_last_iteration_gets_truthful_finalization(tmp_path):
    import asyncio
    ...
    from nanobot.session.turn_continuation import INTERNAL_CONTINUATION_PENDING_META
    ...
    provider.chat_with_retry = AsyncMock(side_effect=[
        ...
        LLMResponse(
            content="The sustained goal is complete and verified.",
            tool_calls=[],
            usage={},
        ),
    ])
    ...
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=None)
    session = loop.sessions.get_or_create("cli:goal")
    session.metadata[GOAL_STATE_KEY] = {
        "status": "active",
        "objective": "Finish and verify the work",
    }
    ...
    outbound = await loop._process_message(message, pending_queue=pending)

    assert outbound is not None
    assert outbound.content == "The sustained goal is complete and verified."
    assert provider.chat_with_retry.await_count == 2
    ...
    assert INTERNAL_CONTINUATION_PENDING_META not in message.metadata
    assert "maximum number of tool call iterations" not in str(session.messages)
