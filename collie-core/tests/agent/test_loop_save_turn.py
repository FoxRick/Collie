...
async def test_websocket_internal_continuation_keeps_single_visible_run(
    ...
    while loop.bus.outbound_size:
        outbound.append(await loop.bus.consume_outbound())
    statuses = [m.event for m in outbound if isinstance(m.event, GoalStatusEvent)]
    assert [m.status for m in statuses] == ["running", "running", "idle"]
    assert isinstance(statuses[0].started_at, float)
    assert statuses[1].started_at == statuses[0].started_at
    turn_end = [m for m in outbound if isinstance(m.event, TurnEndEvent)]
