...
class _FakeWriteTool(Tool):
    ...
    async def execute(self, path: str = "", content: str = "", **kwargs: object) -> str:
        if not content:
            raise RuntimeError("missing required parameter: content")
...
class _FakeEditTool(Tool):
    ...
    async def execute(self, path: str = "", old_text: str = "", new_text: str = "", **kwargs: object) -> str:
        if self._workspace and path:
            ...
            result = old.replace(old_text, new_text)
...
async def test_runner_emits_write_file_diff_from_tool_execution_snapshots(tmp_path):
    ...
    tool = _FakeWriteTool(workspace=tmp_path)
...
async def test_runner_emits_edit_file_diff_from_tool_execution_snapshots(tmp_path):
    ...
    tool = _FakeEditTool(workspace=tmp_path)
...
async def test_runner_marks_file_edit_activity_failed_when_cancelled(tmp_path):
    ...
    class SlowWriteTool(_FakeWriteTool):
