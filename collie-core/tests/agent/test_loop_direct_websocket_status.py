...
@pytest.mark.asyncio
async def test_process_direct_merges_only_explicit_message_metadata(tmp_path) -> None:
    loop = _make_loop(tmp_path)
    loop._connect_mcp = AsyncMock()
    seen: list[dict] = []

    async def _process_message(msg, **_kwargs):
        seen.append(dict(msg.metadata))
        return OutboundMessage(channel=msg.channel, chat_id=msg.chat_id, content=msg.content)
    ...
    await loop.process_direct(
        "objective",
        session_key="collie:goal",
        message_metadata=trusted,
    )
    await loop.process_direct("ordinary", session_key="collie:ordinary")
