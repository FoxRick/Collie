...
def test_load_skill_rejects_traversal_and_foreign_names(tmp_path: Path) -> None:
    workspace = tmp_path / "ws"
    (workspace / "skills" / "helpful").mkdir(parents=True)
    (workspace / "skills" / "helpful" / "SKILL.md").write_text("# Helpful\n", encoding="utf-8")
    outside = tmp_path / "secret.md"
    outside.write_text("secret\n", encoding="utf-8")
    loader = SkillsLoader(workspace, builtin_skills_dir=None)

    assert loader.load_skill("helpful") == "# Helpful\n"
    for name in (
        "../secret.md",
        "..\\secret.md",
        "C:/secret.md",
        ...
        assert loader.load_skill(name) is None, name


def test_load_skill_rejects_names_not_in_skill_dirs(tmp_path: Path) -> None:
    workspace = tmp_path / "ws"
    (workspace / "skills" / "real").mkdir(parents=True)
    (workspace / "skills" / "real" / "SKILL.md").write_text("# Real\n", encoding="utf-8")
    # A sibling SKILL.md that is not inside a named skill directory.
    (workspace / "SKILL.md").write_text("# Root\n", encoding="utf-8")
    loader = SkillsLoader(workspace, builtin_skills_dir=None)
    assert loader.load_skill("real") == "# Real\n"
